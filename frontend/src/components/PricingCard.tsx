      ...
      <ul className="mt-6 space-y-3">
        {features.map((f, i) => (
          <li key={i} className={`flex items-center text-sm ${highlighted ? 'text-blue-100' : 'text-gray-600'}`}>
            <svg className="w-4 h-4 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
            {f}
          </li>
        ))}
      </ul>
      <button className={`mt-8 w-full py-3 rounded-lg font-medium transition ${highlighted ? 'bg-white text-blue-600 hover:bg-blue-50' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
        {cta}
      </button>
    </div>
  );
