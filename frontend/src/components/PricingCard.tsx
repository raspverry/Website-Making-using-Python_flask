...
export function PricingCard({ name, price, period, features, cta, highlighted, comingSoon }: PricingCardProps) {
  ...
  return (
    <div className={`relative rounded-2xl p-8 transition-all duration-200 ${
      ...
      <h3 className={`text-lg font-semibold ${highlighted && !comingSoon ? 'text-white' : 'text-gray-900'}`}>{name}</h3>
      <div className="mt-4 flex items-baseline">
        <span className={`text-4xl font-bold tracking-tight ${comingSoon ? 'text-gray-400' : ''}`}>{price}</span>
        <span className={`ml-1 text-sm ${highlighted && !comingSoon ? 'text-blue-200' : 'text-gray-500'}`}>{period}</span>
      </div>
      <ul className="mt-6 space-y-3">
        {features.map((f, i) => (
          <li key={i} className={`flex items-start gap-2.5 text-sm ${highlighted && !comingSoon ? 'text-blue-100' : 'text-gray-600'}`}>
            <svg className={`w-4 h-4 mt-0.5 shrink-0 ${highlighted && !comingSoon ? 'text-blue-200' : 'text-blue-500'}`} fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
